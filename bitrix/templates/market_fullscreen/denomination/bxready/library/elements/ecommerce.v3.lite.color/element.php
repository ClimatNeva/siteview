...
?>
	...
	<div class="bxr-ecommerce-v3-lite-color" data-uid="<?=$UID?>" data-resize="1" id="<?=$arItemIDs["ID"]?>">
		<div class="bxr-element-container bxr-border-color">
				...
				<a href="<?=$arElement["DETAIL_PAGE_URL"]?>" id="<?=$arItemIDs["ID"]?>" title="<?=$arElement["NAME"]?>">
                        ...
                        <div class="bxr-ra-string">
                            <div class="bxr-element-rating">
                                <div class="bxr-rating-fs" style="background-position: 0 -<?=round($rating/20)*12?>px"></div>
                            </div>
...
<?
$elementDraw->setAdditionalFile("JS", "/bitrix/tools/bxready/library/elements/ecommerce.v3.lite.color/include/script.js", true);
$elementDraw->setAdditionalFile("CSS", "/bitrix/tools/bxready/library/elements/ecommerce.v3.lite.color/include/style.css", false);
?>
<?if ($_REQUEST["bxr_ajax"]):
	...
	if ($resizeIndicator != true){
		...
		<script>
			...
			catalogEcommerceV3LiteColor.resizeVerticalBlock();
