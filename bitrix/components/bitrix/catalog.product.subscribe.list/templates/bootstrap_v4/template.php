...
if(!$arResult['USER_ID'] && !isset($arParams['GUEST_ACCESS'])):?>
	...
	<div class="row" id="catalog-subscriber-identification-form" style="<?=$identificationStyle?>">
		<div class="col-md-8 offset-md-2 col-lg-6 offset-lg-3">
			...
			<div class="row">
				<div class="col catalog-subscriber-identification-form">
					...
					<form method="post">
						...
						<button type="submit" class="btn btn-primary"><?=Loc::getMessage('CPSL_BUTTON_SUBMIT_CODE')?></button>
					</form>
				</div>
			</div>
				...
				<div class="col">
					...
					<form method="post">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" name="userContact" id="contactInputCheck" value="<?=!empty($_GET['contact']) ? htmlspecialcharsbx(urldecode($_GET['contact'])): ''?>">
						</div>
						<div class="form-group">
						...
						<button type="submit" class="btn btn-primary"><?=Loc::getMessage('CPSL_BUTTON_SUBMIT_ACCESS')?></button>
					</form>
				</div>
			</div>
...
<!--		</div>-->
	...
	<?if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS'])) // Simple Product
	{
		...
		<div class="bx_catalog_item_controls">
			<div>
				<a class="btn btn-primary" href="<?=$arItem['DETAIL_PAGE_URL'];?>">
					<?=GetMessage('CPSL_TPL_MESS_BTN_DETAIL');?>
				</a>
			</div>
			<div>
				<a id="<?=$arItemIDs['SUBSCRIBE_DELETE_LINK'];?>" class="btn btn-link" href="javascript:void(0)">
