...
<form id="iblockForm<?=$arParams["IBLOCK_ID"]?>" class='bxr-form-body' name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data">
	...
	<input onclick="BXReady.Market.formRefresh(<?=$arParams["IBLOCK_ID"]?>);return false;" <?if($arParams["HIDE_SUBMIT"] == 'Y') echo 'style="display: none;"'?> class="bxr-color bxr-color-button" id="submitForm_<?=$arParams["IBLOCK_ID"]?>" type="submit" name="iblock_submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
