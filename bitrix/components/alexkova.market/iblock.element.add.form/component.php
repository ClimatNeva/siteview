...
$arParams["NAME_FROM_PROPERTY"] = intval($arParams["NAME_FROM_PROPERTY"]);
...
if($arParams["NAME_FROM_PROPERTY"] && !in_array($arParams["NAME_FROM_PROPERTY"],$arParams["PROPERTY_CODES"]))
	$arParams["PROPERTY_CODES"][] = $arParams["NAME_FROM_PROPERTY"];
if($arParams["NAME_FROM_PROPERTY"] && !in_array($arParams["NAME_FROM_PROPERTY"],$arParams["PROPERTY_CODES_REQUIRED"]))
	$arParams["PROPERTY_CODES_REQUIRED"][] = $arParams["NAME_FROM_PROPERTY"];
...
if ($bAllowAccess)
{
	...
	while ($arProperty = $rsIBLockPropertyList->GetNext())
	{
		if($arProperty["IS_REQUIRED"] == "Y")
			$arParams["PROPERTY_CODES_REQUIRED"][] = $arProperty["ID"];
...
{
	// process POST data
	if (check_bitrix_sessid() && (!empty($_REQUEST["save"]) || !empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		if ($arParams['PERSONAL_DATA'] == "Y"){
			if (htmlspecialchars($_REQUEST['accept_personal']) != 'yes'){
				$arResult["ERRORS"][] = strlen($arParams['PERSONAL_DATA_ERROR'])>0 ? $arParams['PERSONAL_DATA_ERROR'] : GetMessage('IBLOCK_FORM_WRONG_PERSONAL_DATA');
			};
		}
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			$targetUrl = trim(htmlspecialcharsbx($_REQUEST["TARGET_URL"]));
			if(!$targetUrl)
				$targetUrl = $GLOBALS["APPLICATION"]->GetCurPage();
			$arUpdatePropertyValues["TARGET_URL"] = $targetUrl;
			...
			else
			{
				...
				if($arParams["NAME_FROM_PROPERTY"])
				{
					$elementName = $_REQUEST["PROPERTY"][$arParams["NAME_FROM_PROPERTY"]];
					if(is_array($elementName))
						$elementName = current($elementName);
					if(!$elementName)
						$elementName = GetMessage("NEW_IBLOCK_ELEMENT_ADD_FORM_NAME");
					$arUpdateValues["NAME"] = $elementName;
				}
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			if(strlen($arParams["SEND_EVENT"])>0)
				$sendEvent = $arParams["SEND_EVENT"];
			if(!$sendEvent)
				$sendEvent = 'KZNC_NEW_FORM_RESULT';

                        $IBProperty = CIBlockProperty::GetList( array(), array("IBLOCK_ID" => $arUpdateValues['IBLOCK_ID'] ));
                        
                        while($IBPropertyRow = $IBProperty->Fetch()) {
                            $arProperty[$IBPropertyRow['ID']] = $IBPropertyRow['CODE'];
                        }

                        foreach ($arUpdateValues["PROPERTY_VALUES"] as $propId=>$propVal)
			{
				if($propId == 'TARGET_URL')
					$eventFields["TARGET_URL"] = $propVal;
                                else {
                                    if(isset($arProperty[$propId]))
                                        $eventFields[$arProperty[$propId]] = $propVal;
                                }

				$eventFields["PROPERTY_" . $propId] = $propVal;
			}
			$eventFields["IBLOCK_ID"] = $arIBlock["ID"];
			$eventFields["IBLOCK_NAME"] = $arIBlock["NAME"];
                        

                        if(!$eventFields['EMAIL_TO']) {
                            $eventFields['EMAIL_TO'] = "";
                        };
                        
                        CEvent::Send($sendEvent, SITE_ID, $eventFields);

			if($arParams["AJAX"] == 'Y')
			{
				echo "success".$arParams["USER_MESSAGE_ADD"];
				exit();
			}
