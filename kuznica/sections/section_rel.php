...
$file_name = $_SERVER["DOCUMENT_ROOT"].'/kuznica/sections/ps_category.csv';
$catalogIblockId = 12;
...
if (($handle_f = fopen($file_name, "r")) !== FALSE)
{
    ...
    {                    
        ...
        /*$xml = 'group_'.$data_f[0];
        $parent_xml = 'group_'.$data_f[3];
        $ID = false;
        $parent_id = false;

        $arFilterChild = array('IBLOCK_ID' => $catalogIblockId, 'XML_ID'=>$xml);
        $rsSectionsChild = CIBlockSection::GetList(array(), $arFilterChild, false, array('ID','XML_ID'));
        if($arSectionChild = $rsSectionsChild->Fetch())
                    $ID = $arSectionChild['ID'];

        $arFilterParent = array('IBLOCK_ID' => $catalogIblockId, 'XML_ID'=>$parent_xml);
