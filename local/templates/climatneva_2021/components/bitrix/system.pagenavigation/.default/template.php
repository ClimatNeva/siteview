...
else
{
	...
	?>
		<div class="navigation-pages"><?
		if (!$bPrevDisabled) {
			?><a href="<?=$sNextHref;?>" id="<?=$ClientID?>_previous_page">&lt;</a><?
		} else {
			?><span class="navigation-button navigation-disabled">&lt;</span><?
		}
...
if ($APPLICATION->GetCurDir() !== '/' && $checkSingleLinkCounter && $arResult["nEndPage"] > 1) {
    ...
    if ($arResult["NavPageNomer"] == 1) {
        $APPLICATION->AddHeadString('<link rel="next" href="'.$APPLICATION->GetCurDir().'page-2/">');
    } elseif ($arResult["NavPageNomer"] == $arResult["nEndPage"]) {
        $APPLICATION->AddHeadString('<link rel="prev" href="'.$APPLICATION->GetCurDir().'page-'.($arResult["NavPageNomer"]-1).'/">');
    } else {
        $APPLICATION->AddHeadString('<link rel="next" href="'.$APPLICATION->GetCurDir().'page-'.($arResult["NavPageNomer"]+1).'/">');
        $APPLICATION->AddHeadString('<link rel="prev" href="'.$APPLICATION->GetCurDir().'page-'.($arResult["NavPageNomer"]-1).'/">');
    }
}
    */             
//echo "<pre>arResult_template\n",print_r($arResult),"</pre>";
