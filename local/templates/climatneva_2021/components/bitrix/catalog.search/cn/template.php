...
$arSearch = array();
if (sizeof($queryWords)>1) {
    foreach ($queryWords as $word) {
        if (strlen(intval($word)) == strlen($word)) {
            $arSearch[] = intval($word);
        }
    }
} else {
    if (strlen(intval($queryString)) == strlen($queryString)) {
        $arSearch[] = intval($queryString);
    }
}
$arResult["searchable_content"] = array(
    $arSearch,
    $queryString,
    $queryWords,
);
if (sizeof($arSearch)>0) {
    foreach ($arSearch as $item) {
        $res = CIBlockElement::GetList(array(),array("ID" => $item));
        if ($ob = $res->GetNext()) {
            $arElements[] = $ob["ID"];
        }
    }
