...
global $currentPaginationNumber;
...
//	$view = trim(strip_tags($_REQUEST["view"]));
    ...
    if (isset($_REQUEST["section_select"]) && $_REQUEST["section_select"] != "" && $_REQUEST["section_select"] != "all") {
        $GLOBALS["arrFilter"]["IBLOCK_SECTION_ID"] = array($_REQUEST["section_select"]);
        if (CModule::IncludeModule("iblock")) {
            $rsParSect = CIBlockSection::GetByID($_REQUEST["section_select"]);
            if ($arParSect = $rsParSect->GetNext()) {
                $arFilter = array('IBLOCK_ID' => $arParSect['IBLOCK_ID'],'>LEFT_MARGIN' => $arParSect['LEFT_MARGIN'],'<RIGHT_MARGIN' => $arParSect['RIGHT_MARGIN'],'>DEPTH_LEVEL' => $arParSect['DEPTH_LEVEL'],'ACTIVE'=>'Y');
                $rsSect = CIBlockSection::GetList(array('left_margin' => 'asc'),$arFilter);
                while ($arSect = $rsSect->GetNext()) {
                    $GLOBALS["arrFilter"]["IBLOCK_SECTION_ID"][] = $arSect["ID"];
                }
            }
        }
        /*if ($_REQUEST["section_select"]=="106") {
            $GLOBALS["arrFilter"]["IBLOCK_SECTION_ID"] = array(
                $_REQUEST["section_select"],"150","151"
            );
        }*/
    }
    //echo "<pre>arrFilter\n",print_r($GLOBALS["arrFilter"]),"</pre>";
                //IBLOCK_SECTION_ID
                //SECTION_SELECT
	...
	$intSectionID = $APPLICATION->IncludeComponent(
		...
		array(
			...
			"FILTER_NAME" => $arParams["FILTER_NAME"],
			"CACHE_TYPE" => $arParams["CACHE_TYPE"],
			"CACHE_TIME" => "0",//$arParams["CACHE_TIME"], При ненулевом значении слетают link canonical и прочее
...
//			"OFFERS_LIMIT" => $arParams["LIST_OFFERS_LIMIT"],
            ...
            "SHOW_DESCRIPTION_AFTER_SECTION" => $arParams["SHOW_DESCRIPTION_AFTER_SECTION"],
...
//echo "<pre>Title section_vertical: ",$APPLICATION->GetPageProperty('title').'. Страница '.$currentPaginationNumber,"</pre>";
//echo "<pre>Current page: ",$currentPaginationNumber,"</pre>";
/*if ($currentPaginationNumber > 1) { 
    $APPLICATION->SetPageProperty('title',$APPLICATION->GetPageProperty('title').'. Страница '.$currentPaginationNumber);
    $APPLICATION->SetPageProperty('description',rtrim($APPLICATION->GetPageProperty('description'),'.').'. Страница '.$currentPaginationNumber);
}*/
