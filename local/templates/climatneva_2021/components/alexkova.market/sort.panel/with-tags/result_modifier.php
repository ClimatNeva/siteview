...
if (CModule::IncludeModule("iblock")) {
    $dbParentID = CIBlockSection::GetList(array(),array("CODE"=>$arParams["SECTION_CODE"]));
    if ($arPID = $dbParentID->GetNext()) {
        $dbSectList = CIBlockSection::GetList(array("SORT"=>"asc"),array("SECTION_ID"=>$arPID["ID"]));
        while ($arSect = $dbSectList->GetNext()) {
            $arResult["SECTION_LIST"][$arSect["ID"]] = $arSect["NAME"];
        }
    }
}
